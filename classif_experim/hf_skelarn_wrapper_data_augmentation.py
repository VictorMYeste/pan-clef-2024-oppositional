...
from transformers import AutoModelForSequenceClassification, TrainingArguments, Trainer, \
    TextClassificationPipeline, get_scheduler
...
def random_insertion_deletion(sentence, aug_rate=0.1):
    ...
    while i < n:
        ...
        if random.random() < aug_rate:
            new_word = random.choice(list(wordnet.words()))
...
def invert_sentence(sentence, nlp):
    doc = nlp(sentence)
...
def paraphrase_sentence(sentence, model, tokenizer, num_beams=5, num_return_sequences=1, device='cpu'):
...
class SklearnTransformerClassif(SklearnTransformerBase):
    ...
    def _init_model(self, num_classes):
        ...
        # Freeze all layers but the 4 last ones
        for param in self.model.base_model.parameters():
            param.requires_grad = False
        for param in self.model.base_model.encoder.layer[-4:].parameters():
    ...
    def _prepare_dataset(self, X, y):
        ...
        # df['text'] = df['text'].apply(synonyms_augmentation)
        # df['text'] = df['text'].apply(random_insertion_deletion)
        # nlp = spacy.load('en_core_web_sm')
        # nlp = spacy.load('es_core_news_sm')
        # df['text'] = df['text'].apply(invert_sentence, args=(nlp, ))
        tokenizer = transformers.T5Tokenizer.from_pretrained('google/mt5-base')
        model = transformers.T5ForConditionalGeneration.from_pretrained('google/mt5-base')
        df['text'] = df['text'] + " " + df['text'].apply(lambda x: paraphrase_sentence(x, model, tokenizer, 5, 1, self._device))
